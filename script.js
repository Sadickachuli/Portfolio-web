    ...
    images.forEach((image, index) => {
      image.addEventListener('mouseenter', () => {
        if (isImageVisible[index]) {
          image.style.borderRadius = '50%'; // Makes the image circular when the mouse hovers over it
          textDivs[index].style.display = 'none'; // Hides the text-box
          image.style.transform = `scale(${scaleFactor})`; // This will zoom in the image
          image.style.transition = 'transform 0.3s'; // To add a smooth transition effect
        }
      });
